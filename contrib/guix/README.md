# Bootstrappable BitcoinOil Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinOil
Core builds.
...
To build BitcoinOil Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinOil Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinoil-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoinoil-core/guix.sigs).
