...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinoil-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinoil-cli', help='bitcoinoil-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinOil address to which the faucet should send')
...
parser.add_argument('bitcoinoil_cli_args', nargs='*', help='Arguments to pass on to bitcoinoil-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinoil_cli_args == []:
    args.bitcoinoil_cli_args = ['-signet']


def bitcoinoil_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinoil_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinoil_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinoil_cli(['getnewaddress', 'faucet', 'bech32'])
