...
def parse_config_into_btco_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"BitcoinOil Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinoil_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinoil_config.h'), "w", encoding="utf8") as btco_config:
        btco_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='BitcoinOil-core msbuild configuration initialiser.')
    ...
    parse_config_into_btco_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinoil_config.h'), os.path.join(SOURCE_DIR, 'config/bitcoinoil-config.h'))
