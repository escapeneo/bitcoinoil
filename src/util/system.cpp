...
const char * const BITCOINOIL_CONF_FILENAME = "bitcoinoil.conf";
const char * const BITCOINOIL_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinoil-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITCOINOIL_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinOil
    // macOS: ~/Library/Application Support/BitcoinOil
    // Unix-like: ~/.bitcoinoil
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinOil";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinOil";
#else
    // Unix-like
    return pathRet / ".bitcoinoil";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BITCOINOIL_CONF_FILENAME));
}
