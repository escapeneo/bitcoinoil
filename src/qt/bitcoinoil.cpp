...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitcoinOilUnit>("BitcoinOilUnit");
#else
    qRegisterMetaType<BitcoinOilUnit>("BitcoinOilUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoinoil_de.qm (shortcut "de" needs to be defined in bitcoinoil.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoinoil_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoinoil.qrc)
...
static const char* qt_argv = "bitcoinoil-qt";

BitcoinOilApplication::BitcoinOilApplication()
...
void BitcoinOilApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinOilApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinOilGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinOilApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinOilApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinOilApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinOilGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinOilGUI::quitRequested, this, &BitcoinOilApplication::requestShutdown);
...
void BitcoinOilApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinOilApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinOilApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinOilApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinOilApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinOilApplication::handleRunawayException);
    connect(this, &BitcoinOilApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinOilApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinOilApplication::parameterSetup()
{
...
void BitcoinOilApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinOilApplication::requestInitialize()
{
...
void BitcoinOilApplication::requestShutdown()
{
...
void BitcoinOilApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bitcoinoil: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitcoinOilGUI::handlePaymentRequest);
            connect(window, &BitcoinOilGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitcoinOilApplication::handleRunawayException(const QString &message)
{
...
void BitcoinOilApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinOilApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinOilApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinOilGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoinoil);
    Q_INIT_RESOURCE(bitcoinoil_locale);
    ...
    /// 6-7. Parse bitcoinoil.conf, determine network, switch to network specific
    ...
    // bitcoinoil: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
