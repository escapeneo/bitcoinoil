...
/** Call node.updateRwSetting() with BitcoinOil 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitcoinOil 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitcoinoil/bitcoinoil/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitcoinoil 22.x will not try to read these.
...
//! Convert enabled/size values to bitcoinoil -prune setting.
...
//! Get pruning enabled value to show in GUI from bitcoinoil -prune setting.
...
//! Get pruning size value to show in GUI from bitcoinoil -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoinOilUnit")) {
        settings.setValue("DisplayBitcoinOilUnit", QVariant::fromValue(BitcoinOilUnit::BTCO));
    }
    QVariant unit = settings.value("DisplayBitcoinOilUnit");
    if (unit.canConvert<BitcoinOilUnit>()) {
        m_display_bitcoinoil_unit = unit.value<BitcoinOilUnit>();
    } else {
        m_display_bitcoinoil_unit = BitcoinOilUnit::BTCO;
        settings.setValue("DisplayBitcoinOilUnit", QVariant::fromValue(m_display_bitcoinoil_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitcoinoil.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitcoinoil_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinOilUnit>() == m_display_bitcoinoil_unit) return;
    m_display_bitcoinoil_unit = new_unit.value<BitcoinOilUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinOilUnit", QVariant::fromValue(m_display_bitcoinoil_unit));
    Q_EMIT displayUnitChanged(m_display_bitcoinoil_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitcoinoil/bitcoinoil/pull/8273
    ...
    // (https://github.com/bitcoinoil-core/gui/issues/567).
