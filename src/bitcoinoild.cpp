...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bitcoinoild [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitcoinoild -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for bitcoinoild but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect bitcoinoild signal handlers
    noui_connect();
