# CJDNS support in BitcoinOil Core

It is possible to run BitcoinOil Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinOil Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinOil network and individual nodes.
...
before it will work with your BitcoinOil Core node. This is described in steps
...
## Run BitcoinOil Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinOil Core
...
When enabled, this option tells BitcoinOil Core that it is running in an
...
IPv6 local network. This helps BitcoinOil Core perform better address management:
...
CJDNS support was added to BitcoinOil Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinoil-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinOil Core

There are several ways to see your CJDNS address in BitcoinOil Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinoil-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoinoil-cli getpeerinfo`).
