...
Below are some notes on how to build BitcoinOil Core for Windows.

The options known to work for building BitcoinOil Core on Windows are:
...
Note that for WSL the BitcoinOil Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinoil, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinoil`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinoil
