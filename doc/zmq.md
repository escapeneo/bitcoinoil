...
The BitcoinOil Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinoil wire protocol and relay, making
...
The ZeroMQ feature in BitcoinOil Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinoild:
...
For instance:
    ...
    $ bitcoinoild -zmqpubhashtx=tcp://127.0.0.1:219711 \
               -zmqpubhashtx=tcp://192.168.1.2:219711 \
               -zmqpubhashblock="tcp://[::1]:219712" \
               -zmqpubrawtx=ipc:///tmp/bitcoinoild.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in bitcoinoil.conf.
...
From the perspective of bitcoinoild, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinoild directly. Furthermore, no information is
...
using. BitcoinOild appends an up-counting sequence number to each
