BitcoinOil Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/bitcoinoil-core-0.18.0/>
...
`/Applications/BitcoinOil-Qt` (on Mac) or `bitcoinoild`/`bitcoinoil-qt` (on
Linux).
...
recommended to use BitcoinOil Core on unsupported systems.
...
10.10. Additionally, BitcoinOil Core does not yet change appearance when
...
`contrib/init/bitcoinoild.service`, it has been changed to use
`/var/lib/bitcoinoild` as the data directory instead of
`~bitcoinoil/.bitcoinoil`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitcoinoild` will
exist has enough space (check using `df -h /var/lib/bitcoinoild`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:19711:19711` (this is an extra `:19711` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the BitcoinOil Core
...
The systemd init file (`contrib/init/bitcoinoild.service`) has been changed
to use `/var/lib/bitcoinoild` as the data directory instead of
`~bitcoinoil/.bitcoinoil`. This change makes BitcoinOil Core more consistent with
...
`datadir` option in `/etc/bitcoinoil/bitcoinoil.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitcoinoil/bitcoinoil.conf`.


Documentation
-------------

- A new short [document](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/bitcoinoil-conf.md)
  about the `bitcoinoil.conf` file describes how to use it to configure
  BitcoinOil Core.

- A new document introduces BitcoinOil Core's BIP174 [Partially-Signed
  BitcoinOil Transactions
  (PSBT)](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BitcoinOil-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BitcoinOil Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitcoinoil/bitcoinoil/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BitcoinOil Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BitcoinOil Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitcoinoil/bitcoinoil/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitcoinoild with the `-deprecatedrpc=generate`
...
- A new `bitcoinoil-wallet` tool is now distributed alongside BitcoinOil
...
This section describes planned changes to BitcoinOil Core that may affect
other BitcoinOil software and services.

- Since version 0.16.0, BitcoinOil Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BitcoinOil Core 0.20 (expected about
  a year after 0.18), BitcoinOil Core will default to native segwit
  ...
  to bech32 addresses, and if the BitcoinOil Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BitcoinOil Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BitcoinOil Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BitcoinOil Core
...
- A sub-project of BitcoinOil Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BitcoinOil Core.  See their [project
  page](https://github.com/bitcoinoil-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BitcoinOil Core's own implementation, although entropy
  gathered by BitcoinOil Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BitcoinOil Core a
...
- On macOS, BitcoinOil Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitcoinoil_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitcoinoil_server.a` (jnewbery)
...
- #13926 [Tools] bitcoinoil-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitcoinoild (MarcoFalke)
...
- #14521 qt, docs: Fix `bitcoinoil-qt -version` output formatting (hebasto)
...
- #11625 Add BitcoinOilApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitcoinoil_server` (laanwj)
...
- #15154 Configure: bitcoinoil-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitcoinoil-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BitcoinOil Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitcoinoil_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitcoinoil-util-test.py (isghe)
...
- #12775 Integration of property based testing into BitcoinOil Core (Christewart)
...
- #12246 Bugfix: Only run bitcoinoil-tx tests when bitcoinoil-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitcoinoil` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitcoinoil` (MarcoFalke)
...
- #14186 bitcoinoil-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitcoinoil.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitcoinoil.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitcoinoil-conf.md (hebasto)
...
- #15354 Add missing `bitcoinoil-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitcoinoilconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitcoinoil folder on OSX (JimmyMow)
