# PSBT Howto for BitcoinOil Core

Since BitcoinOil Core 0.17, an RPC interface exists for Partially Signed BitcoinOil
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinoil/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinOil transactions that are not fully signed
...
#### Multisig with multiple BitcoinOil Core instances
...
BitcoinOil Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinoil-cli` is used.
...
Later, when *V* BTCO has been received on *Amulti*, and Bob and Carol want to
