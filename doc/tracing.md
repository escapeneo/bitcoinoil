# User-space, Statically Defined Tracing (USDT) for BitcoinOil Core

BitcoinOil Core includes statically defined tracepoints to allow for more
...
**Note**: BitcoinOil Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitcoinOil Core
...
Multiple tools can list the available tracepoints in a `bitcoinoild` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitcoinOil Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitcoinoild
stap net        outbound_message 0x0000000000107c05 /src/bitcoinoild
stap validation block_connected  0x00000000002fb10c /src/bitcoinoild
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitcoinOil Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinoild | grep NT_STAPSDT -A 4 -B 2
