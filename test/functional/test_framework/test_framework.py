...
TMPDIR_PREFIX = "bitcoinoil_func_test_"
...
class BitcoinOilTestMetaClass(type):
    """Metaclass for BitcoinOilTestFramework.

    Ensures that any attempt to register a subclass of `BitcoinOilTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitcoinOilTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitcoinOilTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitcoinOilTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitcoinOilTestFramework(metaclass=BitcoinOilTestMetaClass):
    """Base class for a bitcoinoil test script.

    Individual bitcoinoil test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitcoinoilds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitcoinoilds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitcoinoil-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitcoinoild = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinoild" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinoilcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinoil-cli" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinoilutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinoil-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoinoild = os.getenv("BITCOINOILD", default=fname_bitcoinoild)
        self.options.bitcoinoilcli = os.getenv("BITCOINOILCLI", default=fname_bitcoinoilcli)
        self.options.bitcoinoilutil = os.getenv("BITCOINOILUTIL", default=fname_bitcoinoilutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcoinoilds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BitcoinOilRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoinoild', self.options.bitcoinoild) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bitcoinoil-cli', self.options.bitcoinoilcli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                bitcoinoild=binary[i],
                bitcoinoil_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitcoinoild test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitcoinoild test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoinoild's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitcoinoild=self.options.bitcoinoild,
                    bitcoinoil_cli=self.options.bitcoinoilcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoinoil.conf
    ...
    def skip_if_no_bitcoinoild_tracepoints(self):
        """Skip the running test if bitcoinoild has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoinoild has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoinoild_zmq(self):
        """Skip the running test if bitcoinoild has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoinoild has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitcoinoil-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitcoinoil-wallet has not been compiled")

    def skip_if_no_bitcoinoil_util(self):
        """Skip the running test if bitcoinoil-util has not been compiled."""
        if not self.is_bitcoinoil_util_compiled():
            raise SkipTest("bitcoinoil-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoinoil-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitcoinoil-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bitcoinoil-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitcoinoil-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoinoil_util_compiled(self):
        """Checks whether bitcoinoil-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOINOIL_UTIL")
